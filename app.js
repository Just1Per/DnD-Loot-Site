...
const listRegisteredUsersSecure = httpsCallable(
  functions,
  "listRegisteredUsersSecure"
);

const addExistingUserToCampaignSecure = httpsCallable(
  functions,
  "addExistingUserToCampaignSecure"
);
  ...
  el.innerHTML = myCampaigns.map(c => `
    <div class="campaign-card">
      ...
      <div class="campaign-card-actions">
        <button
          class="save-edit-button btn-sm"
          data-add-member="${c.id}"
          data-campaign-name="${escapeHtml(c.name)}">+ Add Member</button>
        <button class="edit-button btn-sm" data-edit-campaign="${c.id}">Edit</button>
        <button class="cancel-button btn-sm" data-delete-campaign="${c.id}" data-campaign-name="${escapeHtml(c.name)}">Delete</button>
      </div>
    </div>`).join("");

  el.querySelectorAll("[data-add-member]").forEach(btn => {
    btn.addEventListener("click", async () => {
      await openAddCampaignMemberModal(
        btn.dataset.addMember,
        btn.dataset.campaignName
      );
    });
  });
...
let registeredCampaignUsers = [];

async function openAddCampaignMemberModal(campaignId, campaignName) {
  const modal = document.getElementById("campaignMemberModal");
  const list = document.getElementById("registeredUserList");
  const searchInput = document.getElementById("campaignMemberSearch");

  if (!modal || !list || !searchInput) return;

  modal.dataset.campaignId = campaignId;
  document.getElementById("campaignMemberModalTitle").textContent =
    `Add Member — ${campaignName}`;

  searchInput.value = "";
  list.innerHTML = `<p class="player-empty">Loading registered users…</p>`;
  modal.style.display = "flex";

  try {
    const result = await listRegisteredUsersSecure({ campaignId });
    registeredCampaignUsers = result.data?.users || [];
    renderRegisteredUserList();
  } catch (error) {
    console.error("Could not load registered users:", error);
    list.innerHTML = `<p class="player-empty">Could not load registered users.</p>`;
    alert(error.message || "Could not load registered users.");
  }
}
...
function renderRegisteredUserList() {
  const list = document.getElementById("registeredUserList");
  const searchEl = document.getElementById("campaignMemberSearch");
  if (!list || !searchEl) return;

  const search = (searchEl.value || "").trim().toLowerCase();

  const filtered = registeredCampaignUsers.filter(user => {
  ...
  if (!filtered.length) {
    list.innerHTML = `<p class="player-empty">No registered users found.</p>`;
    return;
  }
  ...
  list.innerHTML = filtered.map(user => `
    <div class="registered-user-row">
      <div class="registered-user-info">
        <strong>${escapeHtml(user.name || user.email || "Registered User")}</strong>
        ${user.email ? `<span>${escapeHtml(user.email)}</span>` : ""}
      </div>

      <select class="registered-user-role" data-user-role="${user.uid}">
      ...
      <button
        class="save-edit-button btn-sm"
        data-add-registered-user="${user.uid}">Add</button>
    </div>
  `).join("");

  list.querySelectorAll("[data-add-registered-user]").forEach(button => {
    button.addEventListener("click", async () => {
      const uid = button.dataset.addRegisteredUser;
      const campaignId = document.getElementById("campaignMemberModal")?.dataset.campaignId;
      const roleEl = list.querySelector(`[data-user-role="${uid}"]`);
      const role = roleEl?.value || "player";
      const user = registeredCampaignUsers.find(u => u.uid === uid);

      if (!campaignId || !user) return;

      const displayName = user.name || user.email || "this user";
      if (!confirm(`Add ${displayName} to this campaign as ${role}?`)) return;

      button.disabled = true;
      button.textContent = "Adding…";

      try {
        await addExistingUserToCampaignSecure({
          campaignId,
          targetUid: uid,
          role
        });

        registeredCampaignUsers = registeredCampaignUsers.filter(u => u.uid !== uid);
        renderRegisteredUserList();
        alert(`${displayName} was added to the campaign.`);
      } catch (error) {
        console.error("Add campaign member failed:", error);
        alert(error.message || "Could not add this user.");
...
function initModalListeners() {
  ...
  // Campaign member modal
  document.getElementById("closeCampaignMemberModal")?.addEventListener("click", closeCampaignMemberModal);
  document.getElementById("cancelCampaignMemberModal")?.addEventListener("click", closeCampaignMemberModal);
  document.getElementById("campaignMemberSearch")?.addEventListener("input", renderRegisteredUserList);
