...
async function loadMyPendingInvites() {
  ...
  // Email lookup is the established invitation path and remains authoritative
  ...
  if (emailLower) {
    ...
    emailSnap.docs.forEach(d => inviteMap.set(d.id, { id: d.id, ...d.data() }));
  }

  // UID-targeted invitations are an optional upgrade. If the new Firestore
  // rules have not been deployed yet, this query may be denied. That must not
  // break account startup, so we simply keep the email-based results.
  try {
    const uidSnap = await getDocs(
      query(collection(db, "campaignInvites"), where("targetUid", "==", uid))
    );
    uidSnap.docs.forEach(d => inviteMap.set(d.id, { id: d.id, ...d.data() }));
  } catch (e) {
    console.warn("UID-targeted invitation lookup unavailable; using email invitations only:", e?.message || e);
  }
